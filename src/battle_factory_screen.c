...
static u8 Swap_FindFirstUnlockedEnemyChoice(void);
...
static void Select_CopyMonsToPlayerParty(void)
{
    ...
    for (i = 0; i < FRONTIER_PARTY_SIZE; i++)
    {
        for (j = 0; j < sFactorySelectScreen->offerCount; j++)
        {
            ...
            {
                gParties[B_TRAINER_PLAYER][i] = sFactorySelectScreen->offers[j].monData;
...
static void Swap_Task_OpenSummaryScreen(u8 taskId)
{
    ...
    case STATE_SUMMARY_SHOW:
        ...
        if (sFactorySwapScreen->inEnemyScreen)
            ShowPokemonSummaryScreen(SUMMARY_MODE_NORMAL, gParties[B_TRAINER_OPPONENT_A], sFactorySwapScreen->cursorPos, FRONTIER_PARTY_SIZE - 1, CB2_InitSwapScreen);
        else
...
static void CB2_InitSwapScreen(void)
{
    ...
    case 14:
        ...
        ShowBg(2);
        HideBg(3);
        // Only semi-transparent OBJ (locked enemy choices) should blend.
        // Do not set TGT1_OBJ globally or every sprite gets dimmed.
        SetGpuReg(REG_OFFSET_BLDCNT, BLDCNT_EFFECT_BLEND | BLDCNT_TGT2_BG0 | BLDCNT_TGT2_BG2 | BLDCNT_TGT2_BD);
...
static void Swap_UpdateMonIcons(void)
{
    ...
    for (i = 0; i < FRONTIER_PARTY_SIZE; i++)
    {
        ...
        if (sFactorySwapScreen->enemyIconSpriteIds[i] != SPRITE_NONE)
        {
            ...
            gSprites[spriteId].x = x;
            gSprites[spriteId].y = SWAP_ICON_Y;
            if (sFactorySwapScreen->inEnemyScreen && Swap_AlreadyHasSameSpecies(i))
...
static void Swap_UpdateSwapMapIcons(void)
{
     ...
     && sFactorySwapScreen->cursorPos < FRONTIER_PARTY_SIZE)
    {
        ...
        for (i = 0; i < FRONTIER_PARTY_SIZE; i++)
            species[i] = GetMonData(&gParties[B_TRAINER_PLAYER][i], MON_DATA_SPECIES);
        species[sFactorySwapScreen->playerMonId] = GetMonData(&gParties[B_TRAINER_OPPONENT_A][sFactorySwapScreen->cursorPos], MON_DATA_SPECIES);
    }
...
static void Swap_UpdateBallCursorPosition(s8 direction)
{
    ...
    PlaySE(SE_SELECT);
    prevCursorPos = Swap_GetCursorPos();
    cursorPos = prevCursorPos;

    attempts = FRONTIER_PARTY_SIZE;
    while (attempts-- > 0)
    {
        ...
        if (!sFactorySwapScreen->inEnemyScreen || !Swap_AlreadyHasSameSpecies(cursorPos))
            break;
    }

    if (attempts == 0 && sFactorySwapScreen->inEnemyScreen && Swap_AlreadyHasSameSpecies(cursorPos))
        cursorPos = prevCursorPos;
...
static u8 Swap_FindFirstUnlockedEnemyChoice(void)
{
    ...
    for (i = 0; i < FRONTIER_PARTY_SIZE; i++)
    {
        if (!Swap_AlreadyHasSameSpecies(i))
            return i;
    }
...
static void Swap_InitActions(u8 id)
{
    ...
    case SWAP_ENEMY_SCREEN:
        ...
        if (Swap_AlreadyHasSameSpecies(sFactorySwapScreen->cursorPos))
            sFactorySwapScreen->cursorPos = Swap_FindFirstUnlockedEnemyChoice();
        break;
    }
...
{
    u8 i;
    u16 species = GetMonData(&gParties[B_TRAINER_OPPONENT_A][monId], MON_DATA_SPECIES);

    if (GetActiveFactoryBossId() != FACTORY_BOSS_NONE)
        return FALSE;
