...
// battle_main.c (no header declaration)
...
{
    u8 battleSpeedOption = VarGet(VAR_BATTLE_SPEED); // Option menu stores this in a VAR (not the saveblock).
    if (battleSpeedOption > 3)
        battleSpeedOption = 0;
    ...
    // Option values are 0..3 for 1x..4x.
    return battleSpeedOption + 1;
}
